...
\begin{table}[H]
\centering
\caption{Variance-Covariance Matrix of Reduced-Form Residuals ($\Sigma_u$)}
\label{tab:resid_cov}
\begin{tabular}{lcccc}
\hline
...
PAYEMS\_g   & -0.182425  & 0.010779   & 0.018707   & 0.001362  \\
INDPRO\_g   & -0.946386  & 0.018707   & 0.317218   & -0.002280 \\
PCEPI\_g    & -0.098089  & 0.001362   & -0.002280  & 0.027822  \\
\hline
\end{tabular}
\begin{flushleft}
\footnotesize \textit{Note: The table reports the variance-covariance matrix ($\Sigma_u$) of the reduced-form VAR(1) residuals. Diagonal elements represent the variances of the residuals for each variable, while off-diagonal elements represent the covariances.}
\end{flushleft}
\end{table}
...
\begin{itemize}
    \item The entire structural analysis relies on fragile identification assumptions, specifically the choice to place the policy uncertainty index 
    ...
    \item Additionally, the model relies on outdated estimation data, having been trained entirely on historical figures from 1985 to 2019 to forecast 
    ...
    \item Finally, the uncertainty index itself conflates several different economic measures by bundling media sentiment, expiring tax codes, and 
